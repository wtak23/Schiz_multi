...
%%
% Tak - i modified this to display bytes in MB, rather than KB
%%
...
for ii=1:N
    ...
    S(ii).megabytes=s(ii).bytes/2^20;
    if S(ii).megabytes>=100
        
     memStrings{ii}=num2str(ceil(S(ii).megabytes));
    else 
     memStrings{ii}=num2str(S(ii).megabytes,'%2.3f'); 
    end
...
if ~nargout
  ...
  MegCol=fliplr( char(fliplr('MB'),' ',memStrings{:}));
