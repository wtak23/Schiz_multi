function varargout = tak_showcs3(varargin)
% Function tak_showcs3(V) will display x,y,z cross sections of the volume V
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @tak_showcs3_OpeningFcn, ...
                   'gui_OutputFcn',  @tak_showcs3_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before tak_showcs3 is made visible.
function tak_showcs3_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to tak_showcs3 (see VARARGIN)

% Choose default command line output for tak_showcs3
...
function varargout = tak_showcs3_OutputFcn(hObject, eventdata, handles) 
...
slicex_x=[posx posx;posx posx];
slicex_y=[0 (data.sizes(2)-1);0 (data.sizes(2)-1)]; 
slicex_z=[0 0;(data.sizes(3)-1) (data.sizes(3)-1)];

slicey_x=[0 (data.sizes(1)-1);0 (data.sizes(1)-1)]; 
slicey_y=[posy posy;posy posy]; 
slicey_z=[0 0;(data.sizes(3)-1) (data.sizes(3)-1)];

slicez_x=[0 (data.sizes(1)-1);0 (data.sizes(1)-1)]; 
slicez_y=[0 0;(data.sizes(2)-1) (data.sizes(2)-1)]; 
slicez_z=[posz posz;posz posz];
