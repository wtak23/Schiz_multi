%% gridsearch_NB_PCA_fullRe.m
% (05/10/2014)
%=========================================================================%
% - Try Naive-Bayes method on sMRI dataset using PC loadings
%-------------------------------------------------------------------------%
% - gridsearch over # PCs
...
fsave=true;
%% load data
load sMRI_design_censor_fullRes.mat y

% scale to 0 mean, unit variance?
flag_scale = false;
if flag_scale
    load([get_rootdir,'/data_local/PCA_zscaled_fullRes.mat'],'X')
else
    load([get_rootdir,'/data_local/PCA_zscaled_fullRes.mat'],'X')
end
