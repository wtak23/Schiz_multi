%% save_maskmat_B_cropped_fullRes
% (05/10/2014)
%=========================================================================%
% - save masking matrix B (more precisely, masking vector b)
% - code from may09_get_maskop_cropped.m
%-------------------------------------------------------------------------%
...
load([get_rootdir,'/data_local/mask_cropped_fullRes.mat'],'nii_mask_crop','crop_range')
mask = nii_mask_crop.img;
[nx,ny,nz]=size(mask);

load([get_rootdir,'/data_local/diffmat_C_fullRes.mat'],'C')
load([get_rootdir,'/data_local/A_matrix_cropped_fullRes.mat'],'A')
% return
%%
L = C'*C;
...
% w=ones(p,1);
load sMRI_design_censor_fullRes X
w=X(1,:)';
W=reshape(A*w,NSIZE);
diff_brute=C*w;
...
save([get_rootdir,'/data_local/B_matrix_cropped_fullRes.mat'],'b','mFileName','timeStamp')