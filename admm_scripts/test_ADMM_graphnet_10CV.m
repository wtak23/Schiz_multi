...
%  - the form of the matrix to invert is: (X'*X + 2*I)^-1
%==========================================================================
...
C=tak_diffmat(options.misc.NSIZE,1); % <- C'*C has circulant structure!
options.misc.C=C;
%% begin 10-fold CV
ypredicted = [];
ytrue      = [];
...
%         subplot(121),imcov(w_mat),
%         if idxCV==1,CAXIS1=caxis; else caxis(CAXIS1);end; caxis(caxis./[3 3])
%         wmat_sort=w_mat(idx_sort,idx_sort);
%         subplot(122),imcov(wmat_sort)
%         tak_box_covGroups(gcf, labelCount, yeoLabels)
%         if idxCV==1,CAXIS2=caxis; else caxis(CAXIS2);end; caxis(caxis./[3 3])
%         %%
%     end
%     drawnow
end % <- idxCV loop
...
output=tak_admm_graphnet(X,y,options,K);
% purge
w=output.w;
v1=output.v1;
v2=output.v2;
...
figure,imexp
subplot(121),imcov(v2mat),caxis(caxis./[3 3])
subplot(122),imcov(v2matsort)
tak_box_covGroups(gcf, labelCount, yeoLabels),caxis(CAXIS2); caxis(caxis./[3 3])
...
deg=sum(v2matsort~=0);
hold on
plot(347*ones(1,347)-deg,'k','linewidth',3)
