function output=tak_admm_stl_graphnet(Xlist,ylist,options,Klist)
% output=tak_admm_stl_graphnet(Xlist,ylist,options,Klist)
% (02/22/2014)
%-------------------------------------------------------------------------%
...
dv3=gamma*b+rho*ones(length(b),1); 
...
for k=1:maxiter
    ...
    % update V3
    %-----------------------------------------------------------------%
    V3=rho*bsxfun(@rdivide,(CV4-U3),dv3);
